...
// Was "____chromiumoxide_utility_world___evaluation_script__". That string leaks
// into JS error stack traces (the `sourceUrlLeak` anti-bot detectors look for).
// Emptied so the isolated world's eval script carries no telltale sourceURL.
const EVALUATION_SCRIPT_URL: &str = "";
...
impl FrameManager {
    ...
    pub fn init_commands(timeout: Duration) -> CommandChain {
        ...
        // NOTE: `Runtime.enable` is deliberately NOT sent here. Calling it is the
        // single most reliable CDP automation tell (Cloudflare/DataDome watch for
        // it via the console-serialization side-channel). We instead acquire
        // execution-context ids on demand from `Page.createIsolatedWorld`'s return
        // value (see the MCP crate's isolated-world evaluate helper), which works
        // without enabling Runtime. Consequence: `Runtime.executionContextCreated`
        // events no longer fire, so main_world/secondary_world stay unpopulated —
        // callers must pass an explicit contextId (the MCP crate does).
        CommandChain::new(
            vec![
