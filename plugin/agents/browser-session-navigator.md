...
2. Poll with a **short** timeout in a loop — one long await can exceed the MCP
   client's request timeout and you'd miss the Done signal:
   ```
   let done = false
   while (!done) {
     done = (await await_human_takeover({ token, timeout: 4000 })).completed
   }
   ```
...
- **Be economical.** Batch your reasoning, execute actions sequentially with a
  single confirming snapshot, and don't re-screenshot when a snapshot suffices.
