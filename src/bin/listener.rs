...
use chromiumoxide::cdp::browser_protocol::{
    ...
    page::{
        ...
        EventFrameNavigated, NavigationType,
    },
    target::TargetId,
};
use chromiumoxide::cdp::js_protocol::runtime::{
    DisableParams as RuntimeDisableParams, EnableParams as RuntimeEnableParams,
...
/// per-session override (e.g. a mobile UA the MCP already set).
const LEAKS_CHECK: &str = "(() => { try { const b = navigator.userAgentData && navigator.userAgentData.brands; return navigator.userAgent.indexOf('Headless') >= 0 || (b ? b.some(x => /Headless/.test(x.brand)) : false); } catch (e) { return true; } })()";
...
async fn run_once(browser_url: String, writer: LogWriter) -> Result<()> {
    ...
    // Desktop (Linux) UA override, resolved once and applied to every target that
    // still leaks "Headless" — so tabs opened outside the MCP (takeover "+",
    // popups) get the same masking. None if the version probe fails.
    let ua = user_agent::resolve(&browser, false).await.ok();
    ...
    for page in browser.pages().await.unwrap_or_default() {
        ...
        spawn_target_loop(page, writer.clone(), wired.clone(), ua.clone()).await;
    }
        ...
        tokio::select! {
            Some(event) = attached.next() => {
                ...
                match browser.get_page(TargetId::new(&target_id)).await {
                    Ok(page) => spawn_target_loop(page, writer.clone(), wired.clone(), ua.clone()).await,
...
async fn spawn_target_loop(
    page: Page,
    writer: LogWriter,
    wired: Arc<Mutex<HashSet<String>>>,
    ua: Option<UaOverride>,
) {
    let target_id = page.target_id().inner().to_string();
    tokio::spawn(async move {
        if let Err(err) = target_loop(page.clone(), writer, ua).await {
...
async fn target_loop(page: Page, writer: LogWriter, ua: Option<UaOverride>) -> Result<()> {
    // Per-target setup. Network + Page are enabled unconditionally. Runtime is
    // enabled LATER (just below, once we know the BrowserContext), and only when
    // the session is NOT in stealth — enabling it here would briefly turn Runtime
    // on for every freshly-attached tab, a re-arm window bot-detectors can catch
    // before the stealth poll disables it.
    ...
    // Anti-detection, on EVERY target (incl. tabs opened in the takeover, which
    // bypass the MCP): mask headless tells + block in-page loopback probes. The
    // UA override is gated on a still-leaking-"Headless" check so it doesn't
    // clobber an intentional per-session UA (e.g. mobile) the MCP already set.
    if let Some(ua) = ua.as_ref() {
        let leaks = page
            .evaluate(LEAKS_CHECK)
            .await
            .ok()
            .and_then(|r| r.into_value::<bool>().ok())
            .unwrap_or(true);
        if leaks {
            if let Ok(p) = serde_json::from_value::<SetUserAgentOverrideParams>(json!({
                "userAgent": ua.user_agent,
                "userAgentMetadata": ua.metadata,
    ...
    if let Ok(s) = AddScriptToEvaluateOnNewDocumentParams::builder()
        .source(user_agent::STEALTH_JS)
        .build()
    {
        let _ = page.execute(s).await;
    }
    // Network is enabled above, so setBlockedURLs takes effect on this target.
    if let Ok(b) = serde_json::from_value::<SetBlockedUrLsParams>(
        json!({ "urls": user_agent::BLOCKED_LOOPBACK }),
    ) {
        let _ = page.execute(b).await;
    }
    ...
    // Honor the per-session stealth flag BEFORE enabling Runtime, so a tab that
    // attaches while stealth is active never has Runtime on, even momentarily
    // (no re-arm window). The poll below keeps it in sync if the UI toggles it.
    let stealth_flag = takeover::stealth_path(&ctx_id);
    let mut runtime_on = !tokio::fs::try_exists(&stealth_flag).await.unwrap_or(false);
    if runtime_on {
    ...
    // Console capture toggle: the takeover UI writes a per-session stealth flag
    // (via the daemon). The initial Runtime state was set from this same flag
    // above; here we poll so toggling it in the UI takes effect live. When
    // stealth, Runtime is disabled — stopping console capture AND the
    // console-serialization side-channel bot-detectors use to spot CDP.
    // Network/Page capture is unaffected.
    let mut stealth_poll = tokio::time::interval(Duration::from_secs(2));

    loop {
        tokio::select! {
            _ = stealth_poll.tick() => {
                let stealth = tokio::fs::try_exists(&stealth_flag).await.unwrap_or(false);
                if stealth && runtime_on {
                    let _ = page.execute(RuntimeDisableParams::default()).await;
                    runtime_on = false;
                    tracing::info!(ctx = %ctx_id, "console capture paused (stealth)");
                } else if !stealth && !runtime_on {
                    let _ = page.execute(RuntimeEnableParams::default()).await;
                    runtime_on = true;
                    tracing::info!(ctx = %ctx_id, "console capture resumed");
                }
            }
...
async fn on_request_will_be_sent(state: &Arc<Mutex<TargetState>>, e: &EventRequestWillBeSent) {
    ...
    s.inflight.insert(
        ...
        InflightRequest {
            ...
            status: None,
            // Chrome inlines the request body in `postDataEntries` (base64) for
            // small bodies; decode + concat so callers see e.g. analytics POST
            // payloads. Larger bodies only set `has_post_data` (not fetched here).
            post_data: e.request.post_data_entries.as_ref().and_then(|entries| {
                let mut buf: Vec<u8> = Vec::new();
                for ent in entries {
                    if let Some(b) = ent.bytes.as_ref() {
                        match base64::engine::general_purpose::STANDARD
                            .decode(String::from(b.clone()).as_bytes())
                        {
